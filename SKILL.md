...
9. 工作台/看板/数据分析页第一屏放横幅部件：页面名称 ＋ 一句话介绍，**左对齐**，默认无背景、标题直接坐在页底上靠字号和留白立住；用户要背景时加 `bg-solid` 铺一块干净纯色，禁止网格、环纹、斜纹、点阵这类几何纹理装饰。
10. 横幅介绍学产品官方口吻，说清这页管什么、给谁用，20 字上下，可带轻量价值词，不堆形容词、不加「阵地/平台/门户」这类帽子（例：实现客户、商机与任务排期的集中管理）。
11. 顶部操作按钮走同行右侧的快捷按钮部件（c3「横幅带操作按钮」架构），横幅让宽。
...
| 约束 | 内容 |
| --- | --- |
| 不自造组件 | 伙伴云没有的控件、布局、交互形态一律不画。要画名录（18 类 70 项）里有但结构文件未收录的形态，先告知用户该组件尚未实测采集，确认后按注释里的实测尺寸就近仿写；名录里没有的不画 |
